...
function checkAnswer(evt){
	...
	if(clicked == answer){
			...
			if(spielFortschritt >= rundenProSpiel){
				...
				var ls = localStorage;
				var temp = {};
				//temp["kat"][Kategorie.id+''] = Kategorie;
...
/*
function randomBild(){
	var randomID = Math.floor(Math.random()*katBegriff.length);
	if(katBegriff[randomID].verfuegbar == "false"){
		console.log("randomID vergriffen"+randomID);
		randomID = 0;
		randomBild();
	}
	console.log("randomID: "+randomID);
	
	katBegriff[randomID].choosen = "true";
	bildID = katBegriff[randomID].id;
	return bildID; 
}
*/
/*
